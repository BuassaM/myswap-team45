...
import "https://github.com/BuassaM/Myswap/blob/main/MyswapToken.sol/TokenERC20%20sTandd";
import "https://github.com/BuassaM/Myswap/blob/main/Interfaces/MyswapInterfaceierc20.sol";
import "https://github.com/BuassaM/Myswap/blob/main/Interfaces/MyswapInterfaceFactory.sol";
import "https://github.com/BuassaM/Myswap/blob/main/Interfaces/MyswapInterfaceExchange.sol";


contract MyswapExchange is ERC20 {
  ...
  // Variables
  bytes32 public name;         // Myswap
  bytes32 public symbol;       // MS
  uint256 public decimals;     // 18
  IERC20 token;                // address of the ERC20 token traded on this contract
  IMywapFactory factory;     // interface for the factory that created this contract
  ...
  function setup(address token_addr) public {
    ...
    factory = IMywapFactory(msg.sender);
 ...
 /**
    ...
    private returns (uint256) 
  {
    ...
    require(token.transferFrom(buyer, address(this), tokens_sold));
    uint256 tokens_bought = MyswapExchange(exchange_addr).ethToTokenTransferInput.value(wei_bought)(min_tokens_bought, deadline, recipient);
    emit EthPurchase(buyer, tokens_sold, wei_bought);
    return tokens_bought;
  }
    ...
    private returns (uint256) 
  {
    ...
    require(exchange_addr != address(this) && exchange_addr != address(0));
    uint256 eth_bought = ImyswapExchange(exchange_addr).getEthToTokenOutputPrice(tokens_bought);
    uint256 token_reserve = token.balanceOf(address(this));
    uint256 tokens_sold = getOutputPrice(eth_bought, token_reserve, address(this).balance);
    // tokens sold is always > 0
    require(max_tokens_sold >= tokens_sold && max_eth_sold >= eth_bought);
    require(token.transferFrom(buyer, address(this), tokens_sold));
    uint256 eth_sold = MyswapExchange(exchange_addr).ethToTokenTransferOutput.value(eth_bought)(tokens_bought, deadline, recipient);
    emit EthPurchase(buyer, tokens_sold, eth_bought);
    return tokens_sold;
  }
